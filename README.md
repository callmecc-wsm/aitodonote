...
[打开 Note Note 网页版](https://notenote-mobile-cc.macyu536.chatgpt.site)（私人站点，使用当前账号登录）。手机、平板、电脑浏览器均可使用；源码与运行说明见 [web/README.md](web/README.md)。支持记录、完整草稿、搜索、AI 推进与追问、进展、完成/恢复、延后提醒及安卓备份导入导出。

网页自动回顾和提醒需要页面保持打开；关闭后暂停。真实 AI 需要配置自己的 API Key。浏览器测试使用固定模型响应，不代表真实 iPhone、生产登录或模型服务已经验收。
