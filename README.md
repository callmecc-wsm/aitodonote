...
- **完整草稿**：新记录、编辑原文、补充思考分别自动保存。应用重建后恢复正在写的内容；关闭详情后，卡片提示「有草稿」。草稿不参与 AI 请求。
- **分享接收**：外部分享进入本机待接收区。原来的草稿保留，逐条接收并保存后才成为记录。
- **通知继续聊**：进展通知显示最新摘要。「接着聊」打开对应记录并定位补充框，锁屏仍使用通用提示。
- **阅读与恢复**：长回复在后台刷新时保留阅读位置。导入空库后保持备份里的收件箱顺序。
- **安卓兼容**：旧 WebView 使用软件绘制，处理字体绘制崩溃；输入与详情避开系统栏和键盘。

保留隔天继续研究、最多 3 轮、未读进展、单条搜索开关、安静时段、通知完成与延后操作。

## 下载与安装

1. 登录 GitHub，下载 [NoteNote-Android-APK 测试包](https://github.com/callmecc-wsm/aitodonote/actions/runs/34955111008/artifacts/10389894518)（约 2.4 MB）。
2. 解压得到 `NoteNote-0.3.0-test.apk` 和 `SHA256SUMS.txt`。
3. 在手机上打开 APK 安装。首次打开即可试用记录与本地提醒。

[完整构建与三版安卓测试记录](https://github.com/callmecc-wsm/aitodonote/actions/runs/34955111008) · [验证详情](VALIDATION.md)

附件保留至 2026-12-14。到期后可在 Actions 手动重新构建。
