(function(root) {
  ...
  const status = (t,activeId='',now=Date.now()) => t.done ? '已完成' : t.snooze>now ? '明天再看' : activeId===t.id ? '正在思考' : t.error ? '需要重试' : t.kind==='think' ? (t.needsUser ? '等你聊聊' : t.nextReview ? (t.nextReview<=now?'等待继续研究':'已安排后续研究') : t.rounds>=3 ? '本轮已推进 3 步' : t.lastReview ? '有了新进展' : '待 AI 推进') : t.kind==='action' ? (t.due<=now ? '到时间了' : '等你行动') : '安静保存';
